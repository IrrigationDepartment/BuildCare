  ...
  Widget _buildSchoolCard(BuildContext context, String schoolId, Map<String, dynamic> data) {
    ...
    return Card(
      ...
      child: InkWell(
        borderRadius: BorderRadius.circular(16),
        onTap: () {
          // When navigating from the list, we *know* the data, but it's cleaner
          // to always have the details page fetch its own data for consistency with notifications.
          Navigator.push(
            context,
            MaterialPageRoute(
              builder: (context) => SchoolDetailPage(schoolId: schoolId), // Pass schoolId only
            ),
          );
        },
        child: Padding(
...
// ============================================================================
// REFACTORED: To fetch its own data using schoolId.
class SchoolDetailPage extends StatelessWidget {
  final String schoolId;

  // Constructor now only requires schoolId.
  const SchoolDetailPage({super.key, required this.schoolId});

  // NEW METHOD: Fetches the individual school document from Firestore
  ...
  @override
  Widget build(BuildContext context) {
    // We start by building the overall scaffold.
    return Scaffold(
      backgroundColor: const Color(0xFFF4F6FA),
      appBar: AppBar(
      ...
      // We use a FutureBuilder as the body to get the data from Firestore.
      body: FutureBuilder<DocumentSnapshot>(
        ...
        builder: (context, snapshot) {
          ...
          // Rest of the original layout code, now using 'schoolData' variable.
                    ...
                    children: [
                      ...
                      Card(
                        ...
                        child: Padding(
                          ...
                          child: Column(
                            children: [
                              _buildInfraRow('Electricity', Icons.electrical_services, _getInfraStatus(schoolData, 'electricity')),
                              const Divider(height: 16),
                              _buildInfraRow('Water Supply', Icons.water_drop, _getInfraStatus(schoolData, 'waterSupply')),
                              const Divider(height: 16),
                              _buildInfraRow('Sanitation', Icons.wash, _getInfraStatus(schoolData, 'sanitation')),
                              const Divider(height: 16),
                              _buildInfraRow('Communication', Icons.router, _getInfraStatus(schoolData, 'communication')),
                            ],
                          ),
                        ),
                      ),
  ...
  // REFACTORED: To receive 'schoolData' as a parameter.
  bool _getInfraStatus(Map<String, dynamic> schoolData, String key) {
